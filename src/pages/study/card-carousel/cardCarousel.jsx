                        ...
                        <div className='swiper2'>
                            {
                                list.map((item , index ) => {
                                    ...
                                    if(out ? index < current && current - index <= out : index < current){
                                        ...
                                        origin = 'left center'
                                        zIndex = index +1
                                        transform = `translateX(${(index - current)*offset}px) scale(${1-(current-index)*0.1}) rotateY(80deg)`
                                    }else if(out ?  index > current && index - current <= out : index> current){
                                       // 右边 堆叠
                                        origin = 'right center'
                                        zIndex = list.length - index
                                        transform = `translateX(${(index - current)*offset}px) scale(${1-(index-current)*0.1}) rotateY(-80deg)`
                                    }else if (index === current){
                                        // 当前图片
                                        zIndex = list.length
                                    }
                                    return (
                                        <div className='swiper-item' key={index} style={{transform:transform,zIndex:zIndex,transformOrigin:origin}}>{item}</div>
                                    )
                                })
                            }
                        </div>
